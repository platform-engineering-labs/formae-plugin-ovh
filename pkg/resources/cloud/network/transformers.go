...
var subnetTransformer = &subnetRequestTransformer{}

// subnetPrivateRequestTransformer strips network_id from the request body.
// SubnetPrivate schema uses OVH API field names directly (dhcp, network, noGateway, start, end, region).
// Only network_id needs to be stripped as it's used in the URL path.
