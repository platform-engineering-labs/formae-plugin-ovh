...
// Status delegates to BaseResource. Subnets are created synchronously by
// OVH; the regional propagation race that affects dependent instance
// creates is gated upstream via privateNetworkReadinessProbe instead of
// here, because base.Status's GET-by-ID call doesn't work for subnets
// (OVH only exposes /network/private/{id}/subnet as a list endpoint).
