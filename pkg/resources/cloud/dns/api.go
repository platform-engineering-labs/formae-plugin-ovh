...
func dnsPathBuilder(ctx base.PathContext) string {
	// Zone listing: /domain/zone
	if ctx.Zone == "" {
		return "/domain/zone"
	}

	// Zone read: /domain/zone/{zoneName}
	if ctx.ResourceType == "zone" {
		return fmt.Sprintf("/domain/zone/%s", ctx.Zone)
	}

	// Records/redirections: /domain/zone/{zoneName}/{resourceType}/{id}
