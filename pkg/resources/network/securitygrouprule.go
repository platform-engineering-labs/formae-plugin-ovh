...
// SecurityGroupRule schema and descriptor
var (
	SecurityGroupRuleDescriptor = plugin.ResourceDescriptor{
		Type:         ResourceTypeSecurityGroupRule,
		Discoverable: true,
	}

	SecurityGroupRuleSchema = model.Schema{
		Identifier:   "id",
		Discoverable: true,
		Fields:       []string{"security_group_id", "direction", "ethertype", "protocol", "port_range_min", "port_range_max", "remote_ip_prefix", "remote_group_id", "description"},
...
func (s *SecurityGroupRule) Read(ctx context.Context, request *resource.ReadRequest) (*resource.ReadResult, error) {
	...
	if err != nil {
		return &resource.ReadResult{
			ErrorCode: resources.MapOpenStackErrorToOperationErrorCode(err),
		}, fmt.Errorf("failed to read security group rule: %w", err)
	}
